...
import { CoqWarningQuickFix, QUICKFIX_COMMAND } from "./QuickFixProvider";
import VsCoqToolchainManager, {
  ToolchainError,
  ToolChainErrorCode,
...
export function activate(context: ExtensionContext) {
        ...
        case ToolChainErrorCode.notFound:
          window
            .showErrorMessage(
              ...
              {
                title:
                  "Install the VsCoq language server (Recommended for Coq >= 8.18)",
                id: 0,
              },
              {
                title: "Install VsCoq Legacy (Required for Coq <= 8.17)",
                id: 1,
              },
            )
            .then((act) => {
              if (act?.id === 0) {
                commands.executeCommand(
                  "vscode.open",
                  Uri.parse(
                    "https://github.com/coq-community/vscoq#installing-the-language-server",
                  ),
                );
              }
              if (act?.id === 1) {
          ...
          window
            ...
            .then((act) => {
              if (act?.id === 0) {
                commands.executeCommand(
                  "vscode.open",
                  Uri.parse("https://coq.inria.fr/download"),
                );
              }
              if (act?.id === 1) {
                commands.executeCommand(
  ...
  if (vscoq1) {
    if (vscoq1.isActive) {
      const message =
        "VsCoq2 is incompatible with VsCoq1. it is recommended that you disable one of them.";
      window
        .showErrorMessage(
          message,
          { title: "Disable VsCoq1", id: 0 },
          { title: "Disable VsCoq2", id: 1 },
        )
        .then((act) => {
          if (act?.id === 0) {
  ...
  function registerVscoqTextCommand(
    command: string,
    callback: (textEditor: TextEditor, ...args: any[]) => void,
  ) {
    context.subscriptions.push(
      commands.registerTextEditorCommand("extension.coq." + command, callback),
    );
  }
  ...
  function intializeExtension(serverOptions: ServerOptions) {
    ...
    registerVscoqTextCommand("reset", (editor) => {
      ...
      const req = new RequestType<ResetCoqRequest, ResetCoqResponse, void>(
        "vscoq/resetCoq",
      );
    ...
    registerVscoqTextCommand("query.search", (editor) =>
      launchQuery(editor, "search"),
    );
    registerVscoqTextCommand("query.about", (editor) =>
      launchQuery(editor, "about"),
    );
    registerVscoqTextCommand("query.check", (editor) =>
      launchQuery(editor, "check"),
    );
    registerVscoqTextCommand("query.locate", (editor) =>
      launchQuery(editor, "locate"),
    );
    registerVscoqTextCommand("query.print", (editor) =>
      launchQuery(editor, "print"),
    );
    registerVscoqTextCommand("addQueryTab", () => searchProvider.addTab());
    registerVscoqTextCommand("collapseAllQueries", () =>
      searchProvider.collapseAll(),
    );
    registerVscoqTextCommand("expandAllQueries", () =>
    ...
    registerVscoqTextCommand("stepForward", (editor) => {
      if (workspace.getConfiguration("vscoq.proof").mode === 1) {
        ...
        client.sendNotification("vscoq/stepForward", {
    ...
    registerVscoqTextCommand("stepBackward", (editor) => {
      if (workspace.getConfiguration("vscoq.proof").mode === 1) {
        const textDocument = makeVersionedDocumentId(editor);
        const position = editor.selection.active;
        client.sendNotification("vscoq/stepBackward", {
          textDocument: textDocument,
          position: position,
        });
      } else {
    ...
    registerVscoqTextCommand("showManual", () => {
      commands.executeCommand(
        "simpleBrowser.show",
        "https://coq.inria.fr/doc/master/refman/index.html",
      );
    });
    ...
    client.onNotification(
      ...
      (notification: MoveCursorNotification) => {
        ...
        if (
          workspace.getConfiguration("vscoq.proof.cursor").sticky === true ||
